...
namespace Raspware.GameEngine.Input.Touch
{
	...
	public sealed class Actions : IActions
	{
		...
		private static Dictionary<int, TemporaryTouch> _currentTouches = new Dictionary<int, TemporaryTouch>();
		private Actions(Resolution resolution, IButtons buttons, Layer layer)
		{
			...
			layer.CanvasElement.OnTouchStart = (e) =>
			{
				...
				foreach (var touch in touches)
				{
					...
					_currentTouches.Add(
						touch.Identifier,
						new TemporaryTouch(touch.ClientX, touch.ClientY, resolution.RenderAmount(5))
					);
				}
			};
